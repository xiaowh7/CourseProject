...
## Data sets
The function `DeriveAndWriteDataSets` from the script `run_analysis.R` writes two datasets, `full.csv` and `ans.csv`. The first one, `full` is the result of a cleaning and selection of the data described in [Original data set](#original-data-set). The second one, `ans.csv` is the result of an aggregation of `full.csv`.

### Data set `full.csv`

The dataset `full.csv` is the result of cleaning and selecting data from data from an experiment in which measurements were collected from the accelerometer and gyroscope from the Samsung Galaxy S smartphone used by 30 subjects carrying out a variety of 6 activities.
...
Here follows a description of what `full.csv` looks like.
...
### Data set `ans.csv`
To form the dataset `ans.csv`, the observations from `full.csv` were grouped by subject and activity and then aggregated using the mean function. This yields 180 rows (30 subjects times 6 activities) and 81 columns (1 for subject, 1 for activity plus 79 aggregated features).  
